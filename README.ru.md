...
Здесь мы устройству с адресом 62 включили передачу события при изменении coil (тип 1) регистра 0 с приоритетом 1

### Несколько регистров разных типов (флаг `-T`)

Флаг `-T` позволяет за одну транзакцию настроить события сразу для нескольких регистров разных типов. Флаг можно указывать несколько раз — каждый вызов задаёт блок регистров одного типа.

Формат: `-T "тип:рег=значение[,рег=значение,...]"`

- **тип**: `1` — coil, `2` — discrete, `3` — holding, `4` — input
- **значение**: `0` — выключить, `1` — включить с низким приоритетом, `2` — включить с высоким приоритетом

Если между указанными адресами есть пропуски, они автоматически заполняются нулями (событие выключено).

Пример — включить события для discrete регистров 4 и 6, а также input регистров 464 и 466 на устройстве с адресом 10:
...
Пример — выключить ранее включённое событие для регистра 4:

```
# wb-modbus-scanner -d /dev/ttyRS485-2 -i 10 -T "2:4=0"
```

Флаги `-T` и `-r/-t/-c` являются взаимоисключающими.
