...
Арбитраж - это способ устройствам самим решить, кто будет отвечать на запрос, а также понять, остались ли еще желающие ответить. Арбитраж происходит по уникальному для каждого устройства значению (например, по серийному номеру или `server_id`, в зависимости от команды). Принцип аналогичен арбитражу в CAN шине. Каждое устройство передает один бит своего уникального значения по порядку, начиная с MSB. Когда устройство, передающее рецессивный бит (1), обнаруживает на шине доминантный бит (0), оно проигрывает арбитраж. К завершению передачи уникального значения остается только одно устройство, которое выиграло арбитраж.
...
Библиотека позволяет быстро опрашивать события, возникшие на устройствах без поочередного опроса каждого из них. **Это возможно только если в сети нет устройств с одинаковым server ID**.
...
- (1 байт) минимальный server id устройства с которого начать отвечать
...
- (1 байт) `server_id` устройства, от которого был получен предыдущий пакет с событиями
...
В этот запрос обязательно нужно вставлять флаг для подтверждения получения из предыдущего принятого пакета для этого `server_id`, иначе события не будут сбрасываться.

Пример без ограничения по `server_id` с длиной поля событий 100 байт. Предыдущий пакет с событием был принят от устройства 0x0A с битом 1:
...
Этот пакет отправляется устройством, выигравшим арбитраж (маркер наименьшего приоритета + `server_id`).
...
Если какое-то устройство передает слишком много событий и мешает передавать события остальным, мастер может указать в пакете запроса server id больше, чем у устройства, чтобы пропустить это устройство (вместе со всеми остальными, у которых `server_id` меньше заданного).

### Арбитраж в событиях

Для арбитража при опросе событий используется конкатенация маркера приоритета пакета (_не путать с приоритетом события_) (4 бита) и `server_id` устройства (8 бит).

> Использовать `server_id` вместо серийного номера устройства лучше, потому что арбитраж занимает в 4 раза меньше времени, при этом на момент настройки опроса событий у всех устройств уже ожидаются уникальные значения `server_id`.
...
В реализации опроса событий можно использовать стандартную формулу времени ожидания, в которой ожидается 12 бит арбитража (4 бита маркера приоритета и 8 бит `server_id`). На практике можно использовать хак, который позволяет сократить это время.
...
Фактически, таймаут ответа - это время от отправки запроса до получения первого символа на шине, которым может быть либо доминантный бит арбитража, либо начало сообщения. В арбитраже по `server_id` раньше появятся старшие биты (MSB).

`server_id` в Modbus - это значения от 1 до 247.
...
Когда мастер корректно принял пакет с событиями, он должен подтвердить устройству получение событий. Мастер должен запомнить последнее значение флага в пакете для каждого устройства. В следующем цикле запроса событий в команде запроса `0x10` в поле подтверждения мастер указывает server id устройства и флаг, который был указан в предыдущем принятом пакете с событиями. Этот пакет получают все устройства, даже если они не выиграли арбитраж, и если они видят в поле подтверждения в пакете свой `server_id`, забывают ранее отправленную пачку событий (с соответствующим флагом подтверждения).
...
- (1 байт) server id адрес устройства
...
-> 0A 46 18 15 02 00 04 03 01 00 01 04 01 D0 0A 02 00 02 00 00 00 00 00 00 02 33 A3
    ...
    |  `-- команда                   |   |    |  `-- регистр 464
    `-- server id                     |   |    `-- количество регистров (10)
                                     |   `-- адрес (464)
                                     `-- тип (input)
```
