...
### Функция групповой записи - 0x20

Позволяет за отправку одного пакета выполнить одновременно несколько действий на разных устройствах.

- (1 байт) `0xFD` широковещательный адрес для расширения "быстрый модбас"
- (1 байт) `0x46` команда работы с расширенными функциями
- (1 байт) `0x20` субкоманда - групповая запись регистров
- (1 байт) `slave_id` устройства отправителя, для мастера 0
- (1 байт) флаг изменяющийся при каждом следующем сообщении
- (1 байт) длина поля данных всех команд записи в байтах до контрольной суммы
            блок команд записи для устройства 1
    - (1 байт) `slave_id` устройства
    - (1 байт) длина поля команд записи для данного устройства
                команда записи 1
        - (1 байт) длина данных
        - (1 байт) тип регистра для записи
            0 - coil
            1 - holding
        - (2 байта) адрес регистра (big endian)
        - (0 и более байт) дополнительные данные для записи (формат little endian)
                команда записи 2
                ...
                команда записи n
            блок команд записи для устройства 2
            ...
            блок команд записи для устройства n
- (2 байта) контрольная сумма

Как реализовать подтверждение я пока не знаю. Каждое устройство передает событие если у него есть ошибка. Событие кода ошибки приема предыдущего фрейма.
В результате выполнения команды каждое устройство может иметь статус
    - успех
    - ошибка:
        - не существует объект

    в случае ошибки ее код передается в виде события.
    Приоритет события ниже чем у обычных событий, но выше приоритета сообщения об отсутствии событий. В пакете с сообщением об ошибке передается флаг сообщения в котором возникла проблема.
    В случае успеха передается общий ответ ? или от каждого ? как проверить что устройство обработало команду ?
